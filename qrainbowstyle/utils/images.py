...
def _create_nt_buttons(
    base_svg_path=BUTTONS_NT_PATH, rc_path=RC_PATH, palette=BasePalette
):
    """Create png images from svg files for windows style buttons"""

    sizes = [
        {"ext": ".png", "width": 45, "height": 30},
        {"ext": "@2x.png", "width": 90, "height": 60},
    ]
...
def _create_darwin_buttons(base_svg_path=BUTTONS_DARWIN_PATH, rc_path=RC_PATH):
    ...
    sizes = [
        {"ext": ".png", "width": 32, "height": 32},
        {"ext": "@2x.png", "width": 64, "height": 64},
    ]
...
def create_titlebar_images(
    buttons_svg_path=SVG_PATH, rc_path=RC_PATH, palette=BasePalette
):
...
def create_palette_image(base_svg_path=SVG_PATH, path=IMAGES_PATH, palette=BasePalette):
    """
...
def create_images(base_svg_path=SVG_PATH, rc_path=RC_PATH, palette=BasePalette):
...
def generate_qrc_file(
    ...
    style_prefix="qrainbowstyle",
    rc_path=RC_PATH,
    qrc_path=QRC_FILEPATH,
):
    """
